...
impl RateLimitService for MyRateLimiter {
    ...
    async fn should_rate_limit(
        ...
        // Extract user and group information from request descriptors
        ...
        // Debug logging for descriptors and extracted values
        debug!("Request descriptors: {:?}", req.descriptors);
        info!("EXTRACTION: user_id={:?}, group={:?}", user_id, group);
        ...
        log_openai_debug(&flat_map, Some(hits_addend));

        // Convert to string references for metrics
        let user_id_ref = user_id.as_deref();
        let group_ref = group.as_deref();

        // Debug logging for counters and final values
        debug!("Rate limited response counters: {:?}", rate_limited_resp.counters);
        info!("METRICS: user_id={:?}, group={:?}, limited={}", user_id_ref, group_ref, rate_limited_resp.limited);

        let resp_code = if rate_limited_resp.limited {
            info!("Calling incr_limited_calls_with_user_and_group");
            ...
            self.metrics.incr_limited_hits_with_user_and_group(
                &namespace,
                rate_limited_resp.limit_name.as_deref(),
                user_id_ref,
                group_ref,
            ...
            info!("Calling incr_authorized_calls_with_user_and_group");
...
pub async fn run_envoy_rls_server(
    ...
    let kuadrant_svc = KuadrantService::new(limiter, metrics.clone());
    let kuadrant_server =
